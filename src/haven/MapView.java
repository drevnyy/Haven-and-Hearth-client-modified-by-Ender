...
public class MapView extends Widget implements DTarget, Console.Directory {
    ...
    public boolean mousedown(Coord c, int button) {
		...
		if((Config.pathFinder || ui.modflags() == 3) && (button == 1 || button == 3)){
			if(walker != null){
				walker.forceStopPF = true;
			}
			pathfinder.mainPF.shift=ui.modshift;
			if(button == 1 || pathfinder.mainPF.m_util.checkPlayerCarry()){
				walker = new PathWalker(pathfinder.mainPF.m_util, mousepos, button);
				walker.start();
			}else if(button == 3 && hit != null){
				walker = new PathWalker(pathfinder.mainPF.m_util, hit, button);
				walker.start();
			}
			
			return(true);
		}else if((button == 1 || button == 3) && walker != null){
			if(walker != null){
				walker.forceStopPF = true;
				walker = null;
			}
	...
	{
	g.chcolor(102, 0, 155, 50);
		synchronized(glob.oc) {
		    for(Gob gob : glob.oc) {
			Resource res = gob.getres();
			if(!gob.hide && res != null)
			{
			Resource.Neg neg = gob.getneg();
			if(neg == null)
			    continue;
    ...
    public void draw(GOut og) {
		...
		if(pathfinder.mainPF.publicLineBoolean){
			if(pathfinder.mainPF.publicRects.size()>0)
			drawRectLines(g);
			if(pathfinder.mainPF.publicPoints.size()>0)
			drawRallyLines(g);
		}
	    //###############
  ...
  private void drawPlayerPath(GOut g) {
	...
	if(player != null){
		...
		/*if(pathfinder.mainPF.pathMouse)
		for(Coord c:pathfinder.mainPF.publicPoints){
		    cc = m2s(c).add(oc);
		    g.line(pc, cc, 2);
		    pc = cc;
		}
   ...
   g.chcolor();
	...
	public void drawRectLines(GOut g){
	Coord oc = viewoffset(sz, mc);
	//g.chcolor(0, 0, 200, 255);
	ArrayList<Coord> points = new ArrayList<Coord>();
	boolean sect = false;
	Gob player = glob.oc.getgob(playergob);
	if(player == null) return;
	Coord plyaerC = new Coord(player.getc());
	
	//Line2D.Float line = new Line2D.Float( (float)plyaerC.x, (float)plyaerC.y, (float)mousepos.x, (float)mousepos.y );
	
	//points = pathfinder.mainPF.publicPoints;
	
	ArrayList<Rectangle> allRect = pathfinder.mainPF.publicRects;
	
	if(allRect == null) return;
	
	try{
	for(Rectangle r : allRect){
	Coord a = new Coord(r.x, r.y);
	Coord b = new Coord(r.x + r.width, r.y);
	Coord c = new Coord(r.x, r.y + r.height);
	Coord d = new Coord(r.x + r.width, r.y + r.height);
	
	g.chcolor(0, 0, 200, 255);
	Rectangle2D.Double rs = shrink(r);
	Rectangle2D.Double re = expand(r);
	
	if(re.contains(plyaerC.x, plyaerC.y) ) g.chcolor(0, 200, 0, 255);
	if(rs.contains(plyaerC.x, plyaerC.y) ) g.chcolor(200, 0, 0, 255);
	
	try{
	g.line(m2s(a).add(oc), m2s(b).add(oc),2);
	g.line(m2s(a).add(oc), m2s(c).add(oc),2);
	g.line(m2s(c).add(oc), m2s(d).add(oc),2);
	g.line(m2s(b).add(oc), m2s(d).add(oc),2);
	}
	catch(Exception e){};
	
	//if(line.intersects(r) ) sect = true;
	}
	}
	catch(Exception e){};
	...
	Rectangle2D.Double shrink(Rectangle original){
	double delta = 0.01;
	return new Rectangle2D.Double(
	(double)original.x + delta,
	(double)original.y + delta,
	(double)original.width - delta*2,
	(double)original.height - delta*2
	);
	}
	
	Rectangle2D.Double expand(Rectangle original){
	double delta = 0.01;
	return new Rectangle2D.Double(
	(double)original.x - delta,
	(double)original.y - delta,
	...
	public void drawRallyLines(GOut g){
	Coord oc = viewoffset(sz, mc);
	g.chcolor(255, 0, 64, 255);
	ArrayList<Coord> lines = new ArrayList<Coord>();
	
	lines = pathfinder.mainPF.publicPoints;
	
	boolean first = true;
	Coord c1 = new Coord();
	//Coord plyaerC = new Coord(pathfinder.mainPF.m_hUtil.getPlayerCoord() );
	if(!pathfinder.mainPF.publicLineBoolean)return;
	if(lines == null || lines.size()<2) return;
	for(Coord c : lines){
	if(first){
	first = false;
	c1 = c;
	/*try{g.line(m2s(plyaerC).add(oc), m2s(c).add(oc),2);}
	catch(Exception e){};*/
	}else{
	try{g.line(m2s(c1).add(oc), m2s(c).add(oc),2);}
	catch(Exception e){};
	c1 = c;
	}
	}
